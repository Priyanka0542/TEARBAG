        ...
        {!letter ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center relative z-10 max-w-lg mx-auto">
            ...
            <motion.button 
              ...
              onClick={fetchLetter}
              disabled={loading}
              className="px-8 py-4 bg-primary text-primary-foreground rounded-2xl font-medium shadow-[0_0_30px_-5px_rgba(124,58,237,0.4)] hover:bg-primary/90 transition-all flex items-center gap-3 disabled:opacity-70 disabled:shadow-none"
            >
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Sparkles className="w-5 h-5" />}
              {loading ? 'Writing your letter...' : 'Ask for a Letter'}
            </motion.button>
          </div>
        ) : (
